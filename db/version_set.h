...
//  }
  template <class S>
  void ScoreInsert(int level, const S& container) {
    score_set_[level].insert(container.begin(), container.end());
#if 1
    Log(options_->info_log, "scoreSet add: #%lu@%d", container.size(),
        level+2);
#endif
  }
